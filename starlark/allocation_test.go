...
func TestAllocDeclAndCheckBoundary(t *testing.T) {
	...
	if err := thread.CheckAllocs(allocCap); err != nil {
		t.Errorf("unexpected error: %v", err)
	} else if err := thread.CheckAllocs(starlark.SafeAdd64_(allocCap, 1)); err == nil {
	...
	if err := thread.AddAllocs(allocCap); err != nil {
		...
		if err := thread.AddAllocs(starlark.SafeAdd64_(allocCap, 1)); err == nil {
...
func TestOverflowingPositiveDeltaAllocation(t *testing.T) {
	...
	if err := thread.AddAllocs(math.MinInt64 + 1); err != nil {
	...
	} else if allocs := thread.Allocs(); allocs != math.MaxInt64 {
		t.Errorf("incorrect allocations stored: expected %d but got %d", int64(math.MaxInt64), allocs)
	}
	...
	t.Run("counting", func(t *testing.T) {
		...
		t.Run("Grow", func(t *testing.T) {
			...
			st.RunThread(func(thread *starlark.Thread) {
				...
				if int64(builder.Cap()) != (thread.Allocs() - allocs) {
