...
func int_(thread *Thread, _ *Builtin, args Tuple, kwargs []Tuple) (res Value, err error) {
	...
	if s, ok := AsString(x); ok {
		...
		maxExpectedAllocs := SafeDiv64(SafeAdd64(SafeMul64(int64(len(s)), 6), 7), 8)
		if err := thread.CheckAllocs(maxExpectedAllocs); err != nil {
			return nil, err
		}
...
func zip(thread *Thread, _ *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	...
	if rows >= 0 {
		...
		if err := thread.AddAllocs(SafeAdd64(resultSize, arraySize)); err != nil {
			return nil, err
		}
...
func string_lower(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	...
	if err := thread.AddAllocs(SafeAdd64(bufferSize, StringTypeOverhead)); err != nil {
		return nil, err
	}
...
func string_partition(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	...
	resultSize := SafeAdd64(
		EstimateMakeSize(Tuple{subStringTemplate}, 3),
		EstimateSize(Tuple{}),
	)
	if err := thread.AddAllocs(resultSize); err != nil {
		return nil, err
...
func string_replace(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	...
	maxResultSize := SafeDiv64(SafeMul64(int64(len(recv)+1), int64(len(recv))), int64(max(len(old), 1)))
...
func string_find_impl(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple, allowError, last bool) (Value, error) {
	...
	if i < 0 {
		...
		if last {
			...
			if err := thread.AddSteps(SafeAdd64(int64(i), int64(len(sub)))); err != nil {
				return nil, err
			}
		}
	}
