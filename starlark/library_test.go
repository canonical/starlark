...
func TestBytesCancellation(t *testing.T) {
	bytes, ok := starlark.Universe["bytes"]
	if !ok {
		t.Fatal("No such builtin: bytes")
	}
	...
	t.Run("bytes", func(t *testing.T) {
		...
		st.RunThread(func(thread *starlark.Thread) {
			thread.Cancel("done")
			input := starlark.Bytes(strings.Repeat("foobar", st.N))
			_, err := starlark.Call(thread, bytes, starlark.Tuple{input}, nil)
			if err != nil {
				st.Error("expected noop")
			}
		})
	})
...
func TestDirSteps(t *testing.T) {
	...
	t.Run("with-attrs", func(t *testing.T) {
		...
		for _, input := range inputs {
			st := startest.From(t)
			st.RequireSafety(starlark.CPUSafe)
			st.SetMinSteps(int64(len(input.AttrNames())))
			st.SetMinSteps(int64(len(input.AttrNames())))
			st.RunThread(func(thread *starlark.Thread) {
				for i := 0; i < st.N; i++ {
...
func testWriteValueSteps(t *testing.T, name string, overhead int64, shouldFail bool, otherTests []writeValueStepTest) {
	...
	tests := append([]writeValueStepTest{{
		...
		steps: int64(len("{1: None, 2: }")) + 100 + 2,
	}, {
		name:  "Float",
		...
		steps: int64(len("set([None, ])")) + 100 + 2,
	}, {
		name: "Tuple",
...
func TestFailSteps(t *testing.T) {
	...
	testWriteValueSteps(t, "fail", overhead, true, []writeValueStepTest{{
		...
		steps: int64(len(`b"test"`)),
	}, {
		name:  "Bytes (invalid utf8)",
		input: starlark.Bytes(string([]byte{0x80, 0x80, 0x80, 0x80})),
		steps: int64(len(`b"\x80\x80\x80\x80"`)),
	}})
}
	...
	t.Run("var-size", func(t *testing.T) {
		...
		t.Run("string-special", func(t *testing.T) {
			...
			for _, input := range inputs {
				st := startest.From(t)
				st.RequireSafety(starlark.CPUSafe)
				st.SetMinSteps(int64(len(input)))
				st.SetMaxSteps(int64(len(input)))
				st.RunThread(func(thread *starlark.Thread) {
...
func TestGetattrSteps(t *testing.T) {
	...
	t.Run("dynamic-attr", func(t *testing.T) {
		...
		st := startest.From(t)
		st.RequireSafety(starlark.CPUSafe)
		st.SetMinSteps(int64(len(attrName)))
		st.SetMaxSteps(int64(len(attrName)))
		st.RunThread(func(thread *starlark.Thread) {
			for i := 0; i < st.N; i++ {
...
func TestReversedCancellation(t *testing.T) {
	reversed, ok := starlark.Universe["reversed"]
	if !ok {
		t.Fatal("no such builtin: reversed")
	}
...
func TestStrSteps(t *testing.T) {
	testWriteValueSteps(t, "str", 0, false, []writeValueStepTest{{
		...
		name:  "Bytes (invalid utf8)",
		input: starlark.Bytes(string([]byte{0x80, 0x80, 0x80, 0x80})),
		steps: int64(len("\uFFFD\uFFFD\uFFFD\uFFFD") + len([]byte{0x80, 0x80, 0x80, 0x80})),
	}})
}
...
func TestTupleCancellation(t *testing.T) {
	tuple, ok := starlark.Universe["tuple"]
	if !ok {
		t.Fatal("no such builtin: tuple")
	}
...
func TestDictKeysCancellation(t *testing.T) {
	dict := starlark.NewDict(0)
	dict_keys, _ := dict.Attr("keys")
	if dict_keys == nil {
		t.Fatal("no such method: dict.keys")
...
func TestListIndexCancellation(t *testing.T) {
	st := startest.From(t)
	st.RequireSafety(starlark.TimeSafe)
	st.SetMaxSteps(0)
	st.RunThread(func(thread *starlark.Thread) {
		thread.Cancel("done")
		listElems := make([]starlark.Value, st.N)
		for i := 0; i < st.N; i++ {
			listElems[i] = starlark.MakeInt(i)
		}
		list := starlark.NewList(listElems)
		list_index, _ := list.Attr("index")
		if list_index == nil {
			t.Fatal("no such method: list.index")
		}
...
func TestStringCapitalizeSteps(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			st.SetMinSteps(int64(len(test.output)))
			st.SetMaxSteps(int64(len(test.output)))
			st.RunThread(func(thread *starlark.Thread) {
				for i := 0; i < st.N; i++ {
...
func TestStringFormatSteps(t *testing.T) {
	...
	tests := []struct {
		...
		steps:    int64(len(fmt.Sprintf("{%d}", int64(1<<40)))),
	}, {
		name:     "String",
...
func TestStringJoinSteps(t *testing.T) {
	...
	t.Run("execution", func(t *testing.T) {
		...
		st.SetMinSteps(int64(len("aab") + 1))
		st.SetMaxSteps(int64(len("aab") + 1))
		st.RunThread(func(thread *starlark.Thread) {
			iter := &testIterable{
				maxN: st.N,
				nth: func(_ *starlark.Thread, _ int) (starlark.Value, error) {
...
func testStringPartitionMethodSteps(t *testing.T, name string, fromLeft bool) {
	...
	t.Run("not-present", func(t *testing.T) {
		st := startest.From(t)
		st.RequireSafety(starlark.CPUSafe)
		st.SetMinSteps(int64(len(recv)))
		st.SetMaxSteps(int64(len(recv)))
		st.RunThread(func(thread *starlark.Thread) {
			for i := 0; i < st.N; i++ {
...
func TestStringReplaceSteps(t *testing.T) {
	st := startest.From(t)
	st.RequireSafety(starlark.CPUSafe)
	st.SetMinSteps(int64(len("dead🍖🍖")))
	st.SetMaxSteps(int64(len("dead🍖🍖")))
	st.RunThread(func(thread *starlark.Thread) {
		str := starlark.String(strings.Repeat("deadbeef", st.N))
...
func TestStringSplitlinesSteps(t *testing.T) {
	t.Run("small", func(t *testing.T) {
		...
		st := startest.From(t)
		st.RequireSafety(starlark.CPUSafe)
		st.SetMinSteps(int64(len(str)))
		st.SetMaxSteps(int64(len(str)))
		st.RunThread(func(thread *starlark.Thread) {
			for i := 0; i < st.N; i++ {
...
func TestSetIntersectionCancellation(t *testing.T) {
	t.Run("safety-respected", func(t *testing.T) {
		thread := &starlark.Thread{}
		thread.RequireSafety(starlark.TimeSafe)
...
func TestSetIsSubsetCancellation(t *testing.T) {
	const setSize = 1000
	set := starlark.NewSet(setSize)
	for i := 0; i < setSize; i++ {
		set.Insert(starlark.MakeInt(i))
...
func TestSetIsSupersetCancellation(t *testing.T) {
	const setSize = 1000
	set := starlark.NewSet(setSize)
	for i := 0; i < setSize; i++ {
		set.Insert(starlark.Value(starlark.MakeInt(i)))
...
func testDictlikeIterationResources(t *testing.T, value starlark.Value) {
	...
	st.SetMinSteps(int64(starlark.Len(value)))
	st.SetMaxSteps(int64(starlark.Len(value)))
	st.RunThread(func(thread *starlark.Thread) {
		for i := 0; i < st.N; i++ {
