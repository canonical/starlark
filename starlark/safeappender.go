...
func (sa *SafeAppender) Append(values ...interface{}) error {
	...
	sa.steps = SafeAdd64_(sa.steps, int64(len(values)))

	cap := sa.slice.Cap()
	newSize := sa.slice.Len() + len(values)
	if newSize > cap && sa.thread != nil {
		if err := sa.thread.CheckAllocs(SafeMul64_(int64(newSize), int64(sa.elemType.Size()))); err != nil {
			return err
		}
	}
	slice := sa.slice
	...
	if slice.Cap() != cap && sa.thread != nil {
		...
		sa.allocs = SafeAdd64_(sa.allocs, int64(delta))
...
func (sa *SafeAppender) AppendSlice(values interface{}) error {
	...
	sa.steps = SafeAdd64_(sa.steps, int64(toAppend.Len()))

	len := sa.slice.Len()
	cap := sa.slice.Cap()
	toAppendLen := toAppend.Len()
	if newLen := SafeAdd_(len, toAppendLen); newLen > cap && sa.thread != nil {
		// Consider up to twice the size for the allocation overshoot
		allocation := SafeAdd64_(SafeMul64_(int64(newLen), 2), -int64(cap))
		if err := sa.thread.CheckAllocs(int64(SafeMul64_(allocation, int64(sa.elemType.Size())))); err != nil {
			return err
		}
	}
	...
	if slice.Cap() != cap && sa.thread != nil {
		oldSize := roundAllocSize(SafeMul64_(int64(cap), int64(sa.elemType.Size())))
		newSize := roundAllocSize(SafeMul64_(int64(slice.Cap()), int64(sa.elemType.Size())))
		delta := SafeAdd64_(newSize, -oldSize) // Precondition: oldSize is not MaxInt64.
		sa.allocs = SafeAdd64_(sa.allocs, delta)
		if err := sa.thread.AddAllocs(delta); err != nil {
			return err
		}
	}
